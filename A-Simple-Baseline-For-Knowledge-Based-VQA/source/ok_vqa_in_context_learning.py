...
def val_in_context_learning_ok_vqa(llama_model, 
  ...
  for i in tqdm(range(val_annotations_df.shape[0])):
    ...
    for k in range(k_ensemble): # we use k promts for each test sample
      ...
      outputs = llama_model.generate(input_ids, max_length=prompt_tokens + 5, num_beams=2,pad_token_id=llama_tokenizer.pad_token_id, return_dict_in_generate=True, output_scores=True, num_return_sequences=1,
...
def val_in_context_learning_ok_vqa_with_mcan(llama_model, 
  ...
  for i in tqdm(range(val_annotations_df.shape[0])):
    ...
    for k in range(k_ensemble): # we use k promts for each test sample
      ...
      # Generate 
      input_ids = inputs.input_ids.to(device)
      outputs = llama_model.generate(input_ids,pad_token_id=llama_tokenizer.pad_token_id, max_length=prompt_tokens + 5, num_beams=2, return_dict_in_generate=True, output_scores=True, num_return_sequences=1,
                                     do_sample=False, temperature = 1.0, top_p = 1.0)
